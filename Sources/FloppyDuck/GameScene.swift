...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    private func createButton(text: String, position: CGPoint) -> SKNode {
        ...
        // Create a rounded rect path for the button
        let roundedRect = SKShapeNode(rectOf: CGSize(width: 220, height: 60), cornerRadius: 10)
        roundedRect.fillColor = .systemBlue
        roundedRect.strokeColor = .clear
        roundedRect.alpha = 0.8
        buttonNode.addChild(roundedRect)
    ...
    private func setupPlayer(node: SKNode, isPlayerOne: Bool) {
        ...
        if isPlayerOne {
            ...
            if let scoreLabel = player2ScoreLabel {
                scoreLabel.fontSize = 60
                scoreLabel.position = CGPoint(x: frame.width/4, y: frame.height - 100)
                scoreLabel.text = "0"
